...
routes.get('/activity/user/finished', async (req, res) => {
    ...
    const ssql1 = "select a.id, a.author_uid, a.title, a.type_activity, aqu.created as replied_in, a.image_reference, a.image_url, a.difficulty_level from activity_question_users aqu inner join activity a on(aqu.activity_id = a.id) where a.excluded is null and aqu.user_uid = '" + firebase_uid + "' and aqu.display_to_user = '1'";
...
routes.post('/activity', async (req, res) => {
    const firebase_uid = req.header('firebase_uid');
    const { title, password, type_activity, image_reference, image_url, image_type, image_size_wh, difficulty_level } = req.body;
    ...
    if(!title || !type_activity || !image_reference || !image_url || !image_type || !image_size_wh || !difficulty_level){
...
routes.get('/activity/:id', async (req, res) => {
    ...
    try{
        ...
        await prisma.$queryRawUnsafe("select a.id, a.author_uid, a.title, a.with_password, a.password, u.name || ' ' || u.last_name as name, a.type_activity, a.image_reference, a.image_url, a.difficulty_level from activity a inner join users u on(a.author_uid = u.firebase_uid) where a.id = '" + id_ascii + "' and a.excluded is null limit 1")
        .then((json) => {
...
routes.get('/activity/:id/users/concluded', async (req, res) => {
    ...
    var ssql = "select aqu.id, aqu.activity_id, u.id, aqu.user_uid, u.email, u.name || ' ' || u.last_name as full_name, aqu.value, aqu.created, u.image_reference, u.image_url from activity_question_users aqu inner join users u on u.firebase_uid = aqu.user_uid where aqu.activity_id = '" + id + "'"
